...
int do_insert(char *img_buffer, size_t im_size, char *img_id, struct imgst_file *im_file)
{
    ...
    strncpy(im_file->metadata[index].img_id, img_id, 127);
    ...
    if (im_file->metadata[index].offset[RES_ORIG] == 0) {
        ...
        fwrite(img_buffer, im_size, 1, im_file->file);
    }
