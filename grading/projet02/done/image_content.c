...
int inp_to_out_buffer(void ** output_buffer, void ** input_buffer, struct imgst_file *im_file, uint64_t im_size_orig, int res, size_t* im_size_new)
{
...
int lazily_resize(int res, struct imgst_file *im_file, size_t index)
{
    if (res == RES_ORIG) {
        return ERR_NONE;
    } else if (im_file == NULL || (res < 0 || NB_RES <= res) || index >= im_file->header.num_files) {
...
int get_resolution(uint32_t *height, uint32_t *width, const char *image_buffer, size_t image_size)
{
    ...
    *width = vips_image_get_width(im_input);
    *height = vips_image_get_height(im_input);
    return ERR_NONE;
}
